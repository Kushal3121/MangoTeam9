...
window.onerror = function mangoHandler(desc, page, line) {
  BrowserDetect.init();
  if (
    checkCombo(BrowserDetect.browser, BrowserDetect.version, BrowserDetect.OS)
  ) {
    MiscDwr.jsError(
      desc,
      page,
...
mango.header.onLoad = function () {
  if (dojo.render.html.ie)
    mango.header.evtVisualizer = new IEBlinker(
      $('__header__alarmLevelDiv'),
      500,
      200
    );
  else
    mango.header.evtVisualizer = new ImageFader(
      $('__header__alarmLevelDiv'),
      75,
      0.2
    );
    ...
    if (result.error) {
      ...
      if (result.relatedList && result.relatedList.length > 0) {
        ...
        for (var i = 0; i < result.relatedList.length; i++)
          content +=
            "<a class='ptr' onclick='helpImpl(\"" +
            result.relatedList[i].id +
            '");\'>' +
            result.relatedList[i].title +
            '</a><br/>';
        content += '</p>';
      }
      if (result.lastUpdated)
        content +=
          '<p>' +
          mango.i18n['js.help.lastUpdated'] +
          ': ' +
          result.lastUpdated +
          '</p>';
...
function SoundPlayer() {
  ...
  this.setMute = function (muted) {
    if (muted != this.mute) {
      this.mute = muted;
      if (this.soundId) {
        if (muted) this._stopRepeat(this.soundId);
        else this._repeat();
      }
    }
  };
  ...
  this._repeat = function () {
    ...
    else setTimeout(self._repeat, 500);
  };
...
var BrowserDetect = {
  init: function () {
    ...
    this.OS =
      ...
      'an unknown OS (' + navigator.userAgent + ')';
  },
...
function checkCombo(browser, version, os) {
  if (browser == 'Firefox' && version > 1) return true;
  if (browser == 'Explorer' && version > 6 && os == 'Windows') return true;
  if (browser == 'Chrome') return true;
  if (browser == 'Safari' && version > 3 && os == 'Mac') return true;
  if (browser == 'Safari' && version > 3 && os == 'Android') return true;
  return false;
}
