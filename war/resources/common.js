...
mango.longPoll.start = function () {
  MiscDwr.initializeLongPoll(
    mango.longPoll.pollSessionId,
    mango.longPoll.pollRequest,
    mango.longPoll.pollCB
  );
...
mango.longPoll.pollCB = function (response) {
  ...
  if (response.viewStates) mango.view.setData(response.viewStates);

  if (typeof response.pendingAlarmsContent != 'undefined')
    updatePendingAlarmsContent(response.pendingAlarmsContent);

  if (response.customViewStates) mango.view.setData(response.customViewStates);
...
function dump(o) {
  for (var p in o) dojo.debug(p + '=' + o[p]);
}
...
function display(node, showNode, styleType) {
  if (showNode) show(node, styleType);
  else hide(node);
}
...
function findRelativeAncestor(node) {
  ...
  while ((node = node.parentNode)) {
    ...
    if (pos == 'relative' || pos == 'absolute') return node;
  }
  return null;
}
...
function IEBlinker(
  /*element*/ node,
  /*milliseconds*/ onTime,
  /*milliseconds*/ offTime
) {
  this.target = node;
  this.on = onTime;
  if (!this.on) this.on = 700;
  this.off = offTime;
  if (!this.off) this.off = 300;
  ...
  this.start = function () {
    ...
    if (this.state) showLayer(this.target);
    else hideLayer(this.target);
    this.timeoutId = setTimeout(
      dojo.lang.hitch(this, 'start'),
...
function startIEBlinker(
  ...
  if (node.blinker) stopIEBlinker(node);
...
function ImageFader(
  ...
  this.start = function () {
    ...
    if (op >= 1) this.increasing = false;
    else if (op <= 0) this.increasing = true;

    if (this.increasing) op += this.step;
    else op -= this.step;
...
function startImageFader(node, disableOnclick) {
  if (disableOnclick) this.disableOnclick(node);

  node = getNodeIfString(node);
  var fader = new ImageFader(node);
  if (node.fader) stopImageFader(node);
  node.fader = fader;
  fader.start();
}
...
function updateTemplateNode(elem, replaceText) {
  ...
  for (i = 0; i < elem.attributes.length; i++) {
    if (
      ...
      elem.attributes[i].value.indexOf('_TEMPLATE_') != -1
    )
      elem.attributes[i].value = elem.attributes[i].value.replace(
        /_TEMPLATE_/,
        replaceText
      );
  }
...
function getElementsByMangoName(node, mangoName, result) {
  ...
  if (node.mangoName == mangoName) result[result.length] = node;
...
function getMangoId(node) {
  while (!node.mangoId) node = node.parentNode;
  return node.mangoId;
}
...
function setAlarmLevelImg(alarmLevel, imgNode) {
  if (alarmLevel == 0)
    updateImg(
      ...
      mango.i18n['common.alarmLevel.none'],
      false
    );
    ...
    updateImg(
      ...
      mango.i18n['common.alarmLevel.info'],
      true
    );
    ...
    updateImg(
      ...
      mango.i18n['common.alarmLevel.urgent'],
      true
    );
    ...
    updateImg(
      ...
      mango.i18n['common.alarmLevel.critical'],
      true
    );
    ...
    updateImg(
      imgNode,
      'images/flag_red.png',
      mango.i18n['common.alarmLevel.lifeSafety'],
      true
    );
  else updateImg(imgNode, '(unknown)', '(unknown)', true);
}

function setAlarmLevelText(alarmLevel, textNode) {
  if (alarmLevel == 0) textNode.innerHTML = '';
  else if (alarmLevel == 1)
  ...
  else textNode.innerHTML = 'Unknown alarm level: ' + alarmLevel;
}
...
function setUserImg(admin, disabled, imgNode) {
  if (disabled)
    updateImg(
      imgNode,
      'images/user_disabled.png',
      mango.i18n['common.disabled'],
      true
    );
  else if (admin)
    updateImg(
      ...
      mango.i18n['common.administrator'],
      true
    );
  else
    updateImg(
...
function setScheduledEventImg(disabled, imgNode) {
  if (disabled)
    updateImg(
      imgNode,
      'images/clock_disabled.png',
      mango.i18n['js.disabledSe'],
      true
    );
  else
    updateImg(
...
function setCompoundEventImg(disabled, imgNode) {
  if (disabled)
    updateImg(
      imgNode,
      'images/multi_bell_disabled.png',
      mango.i18n['js.disabledCed'],
      true
    );
  else
    updateImg(
      imgNode,
      'images/multi_bell.png',
      mango.i18n['compoundDetectors.compoundEventDetector'],
      true
    );
}
...
function setPointLinkImg(disabled, imgNode) {
  if (disabled)
    updateImg(
      ...
      mango.i18n['js.disabledPointLink'],
      true
    );
  else
    updateImg(
      imgNode,
      'images/link.png',
      mango.i18n['pointLinks.pointLink'],
      true
    );
}
...
function togglePanelVisibility(img, panelId, visTitle, invisTitle) {
  var visible = true;
  if (!img.minimized) visible = false;
...
function togglePanelVisibility2(img, panelId, visTitle, invisTitle) {
  var visible = true;
  if (img.minimized == false) visible = false;
  togglePanelVisibilityImpl(img, panelId, visTitle, invisTitle, visible);
}

function togglePanelVisibilityImpl(
  img,
  panelId,
  visTitle,
  invisTitle,
...
function getElement(arr, id, idName) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (arr[i][idName] == id) return arr[i];
  }
  return null;
}
...
function writeImage(id, src, png, title, onclick) {
  ...
  if (png && !src) result += ' src="images/' + png + '.png"';
  if (title) result += ' alt="' + title + '" title="' + title + '"';
...
function hideContextualMessages(parent) {
  ...
  for (var i = 0; i < nodes.length; i++) dojo.html.hide(nodes[i]);
}
...
function showDwrMessages(
  /*DwrResponseI18n.messages*/ messages,
  /*tbody*/ genericMessageNode
) {
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    if (m.contextKey) {
      ...
      if (!node) {
        field = $(m.contextKey);
        if (field) node = createContextualMessageNode(field, m.contextKey);
        else alert('No contextual field found for key ' + m.contextKey);
      }
    ...
    } else genericMessages[genericMessages.length] = m.genericMessage;
  }

  if (genericMessages.length > 0) {
    if (!genericMessageNode) alert('generic messages node not defined');
...
function setSilenced(eventId, silenced) {
  ...
  if (silenced)
    ...
    updateImg(
      imgNode,
      'images/sound_none.png',
      mango.i18n['events.silence'],
      true,
      'inline'
    );
}
...
function ackEvent(eventId) {
  ...
  updateImg(
    imgNode,
    'images/tick_off.png',
    mango.i18n['events.acknowledged'],
    true,
    'inline'
  );
...
mango.share.addUserToShared = function () {
  ...
  if (userId)
    mango.share.dwr.addUpdateSharedUser(
      userId,
      1 /* ShareUser.ACCESS_READ */,
      mango.share.writeSharedUsers
    );
};
  ...
  if (sharedUsers.length == 0) {
    ...
    dwr.util.addRows(
      ...
      [
        ...
        function (data) {
          return (
            "<img src='images/bullet_delete.png' class='ptr' " +
            "onclick='mango.share.removeFromSharedUsers(" +
            data.userId +
            ")'/>"
          );
        },
...
mango.share.updateUserList = function (sharedUsers) {
  ...
  for (var i = 0; i < mango.share.users.length; i++) {
    ...
    if (!found) availUsers.push(mango.share.users[i]);
  }
